...
const PlacedSlider = styled(AnimatedInputSlider)`
  ...
  top: 500px;
`
export const Applet04503Ge: React.FC<{
  ...
  const onSliderChange = (value: number) => {
    ...
    ggbApiRight.current.setValue('t', value / 100)
    if (value === 100) {
      setShowHands((v) => {
        return v == 0 || v == 1 ? 1 : 2
      })
  ...
  useEffect(() => {
    ...
    ggbApiRight.current.setValue('op' + option, 1)
    ggbApiLeft.current.setValue('op' + option, 1)
  }, [activeToggle])
  return (
    <AppletContainer
      {...{
    ...
    >
      <TextHeader
        text="Unfold the cube and observe its nets."
      ...
      {ggbLoaded && (
        <ToggleGroup noOfChildren={11} onChange={handleToggleChange} disabled={resetSlider} />
      )}
