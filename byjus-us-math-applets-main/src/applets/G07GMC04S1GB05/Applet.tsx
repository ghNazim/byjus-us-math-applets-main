...
const ColoredSpan = styled.span<{ bgColor: string; color: string }>`
  ...
  margin: 0 5px;
`
export const AppletG07GMC04S1GB05: FC<{
    ...
    (api: GeogebraAppApi | null) => {
      ...
      ggbApi.current.registerClientListener((event: any) => {
        ...
        if (event[0] == 'dragEnd' && event[1] == "B''") {
          ...
          interaction('drop')
          setHelper(2)
          setAngAOB(ggbApi.current.getValueString('BlueAngle'))
          setAngBOC(ggbApi.current.getValueString('OrangeText'))
        }
      })
    },
    [ggbApi],
  ...
  return (
    <AppletContainer
      {...{
        aspectRatio: 0.9,
        borderColor: '#F6F6F6',
        id: 'g07-gmc04-s1-gb05',
        onEvent,
        className,
      }}
    >
      <TextHeader
        text="Explore the relation between the angle and its supplement."
        backgroundColor="#F6F6F6"
        buttonColor="#1A1A1A"
      />
      <GGB materialId="gyzqpu92" onApiReady={onGGBHandle} />
