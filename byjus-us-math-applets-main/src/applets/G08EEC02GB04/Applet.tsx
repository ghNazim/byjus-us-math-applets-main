...
function getOrderedPrimePairs(
  factors: number[],
): [boolean, Array<[number, number] | number>, number] {
  ...
  for (const [factor, count] of countMap) {
    ...
    while (i > 0) {
      ...
      pairs.push([factor, factor])
      pairCount++
    }
  }

  return [isPerfectSquare, pairs, pairCount]
}
...
const AnswerInputOnboarding = styled(OnboardingAnimation).attrs({ type: 'click' })`
  ...
  z-index: 1;
`
export const AppletG08EEC02GB04: FC<{
  ...
  const [isSquareCheck, groupedFactors] = (factorTreeRef.current &&
    getOrderedPrimePairs(factorTreeRef.current.expandedFactors)) ?? [false, []]
  ...
  useEffect(() => {
    ...
    if (resolvedInputValue > 4 && resolvedInputValue < 300 && isPrime) {
      setErrorCase(false)
      setNotPrime(true)
      setNotInRange(false)
    }
    if (resolvedInputValue > 300) {
      setNotInRange(true)
    }
    if (!isPrime && resolvedInputValue < 300) {
      setErrorCase(false)
      setNotPrime(false)
      setNotInRange(false)
    }
  ...
  return (
    ...
    >
      ...
      <TextBox isVisible={!errorCase && (notInRange || notPrime || isNaN(resolvedInputValue))}>
        <PageText>Enter a composite number betweeen 4 - 300.</PageText>
      </TextBox>
      ...
      {treeComplete && !errorCase && !notPrime && !notInRange && !isNaN(resolvedInputValue) && (
        <>
          ...
          {isSquareCheck ? (
            <EndText>
              These prime factors can be grouped in pairs. <br />
              {resolvedInputValue} is a perfect square.
            </EndText>
          ) : (
            <EndText>
              These prime factors cannot be grouped in pairs.
              <br /> {resolvedInputValue} is not a perfect square.
            </EndText>
          )}
        </>
      )}
