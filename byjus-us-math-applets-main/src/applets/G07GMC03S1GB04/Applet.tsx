...
const AppletOnboarding = styled(OnboardingAnimation)<{ left: number; top: number; type: string }>`
  ...
  width: 400px;
`
export const AppletG07GMC03S1GB04: FC<{
  ...
  const texts = [
    ...
    'Connect the dots to draw the included side of length 8 cm.',
    ...
    `Join the two points.`,
    'Now, let’s proceed to the next step.',
    `Measure the angle ${toggleNum === 60 ? '30' : '60'}° from vertex B to mark it as other side.`,
    `Join the two points.`,
    `Great! You constructed a triangle with
    60°, 30° and an included side of 8 cm.`,
  ]
  ...
  return (
    <AppletContainer
      {...{
        aspectRatio: 0.9,
        borderColor: '#f6f6f6',
        id: 'g07-gmc03-s1-gb04',
        onEvent,
        className,
      }}
    >
      <TextHeader
        text="Construct a triangle with 60°, 30° and
an included side of 8 cm."
        backgroundColor="#f6f6f6"
        buttonColor="#1a1a1a"
      />
      <StyledGeogebra materialId="v8bmvfak" onApiReady={onApiReady} />
      {ggbLoaded && (
        <>
