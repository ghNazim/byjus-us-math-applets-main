...
const InputContainer: FC<InputContainerProps> = ({ onChangeValue1, onChangeValue2, ...props }) => {
  return (
    <Container className={props.className}>
      <InputField max={99} onChange={(val) => onChangeValue1(val)} placeHolder="" state="default" />
      <InputField max={99} onChange={(val) => onChangeValue2(val)} placeHolder="" state="default" />
    </Container>
  )
}
