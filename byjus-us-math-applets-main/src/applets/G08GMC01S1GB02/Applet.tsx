...
const HandPlayer = styled(Player)`
  ...
  pointer-events: none;
`
export const AppletG08GMC01S1GB02: FC<{
  ...
  const onGGBHandle = useCallback(
    (api: GeogebraAppApi | null) => {
      ...
      api.registerObjectUpdateListener('G1', () => {
        if (api.getVisible('G1')) setPageNum(2)
      })
      api.registerObjectUpdateListener('redbg', () => {
        if (api.getVisible('redbg')) setPageNum(1)
      })
    },
    [ggbApi],
  )
  ...
  return (
    ...
    >
      <TextHeader
        text="Determine if the given image is
        same as the original figure using transformations."
        backgroundColor="#f6f6f6"
        buttonColor="#1a1a1a"
      />
      <GGB materialId="yg5pdtxc" onApiReady={onGGBHandle} />
      <HelperText>
        <Text>
          {pageNum == 0 && 'Choose a transformation to align the triangles.'}
          {pageNum == 1 &&
