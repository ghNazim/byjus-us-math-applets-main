...
const ToggleSquareBtn = styled.button<{ checked: boolean }>`
  ...
  padding: 2px 50px;
  background: #ffffff;
  border: ${(props) => (props.checked ? '1px solid #1a1a1a' : ' 1px solid #c7c7c7;')};
  box-shadow: ${(props) => (props.checked ? 'none' : 'inset 0px -4px 0px #C7C7C7')};
  border-radius: 12px;
  transition: 0.3s ease-out;
  scale: ${(props) => (props.checked ? 1.1 : 1)};
  .icon-outer {
  ...
  &:hover {
    scale: ${(props) => (props.checked ? 1.09 : 1.05)};
...
const ToggleImg = styled.img<{ checked: boolean }>`
  max-width: ${(props) => (props.checked ? '160%' : '140%')};
  width: auto;
  height: auto;
  border: none;
  cursor: pointer;
