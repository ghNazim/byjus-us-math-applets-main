...
const trueText = (variable: string, answer: number) => {
  return (
    <div>
      ...
      Hence, we can say that the value of {variable} is {answer}.
    </div>
  )
}
...
const questionTwo: IQuestion = {
  variable: 'x',
  evalFn: (SliderValues: ISliderValues) => {
    const lhsVal = 2 * SliderValues.lhs + 4
    const rhsVal = 12
    if (lhsVal === rhsVal) {
      {
        return {
...
export const questionFour: IQuestion = {
  variable: 'x',
  evalFn: (SliderValues: ISliderValues) => {
    const lhsVal = 3 * SliderValues.lhs - 5
    const rhsVal = 10
    if (lhsVal === rhsVal) {
      {
        return {
...
const questionFive: IQuestion = {
  variable: 'x',
  evalFn: (SliderValues: ISliderValues) => {
    const lhsVal = 4 * SliderValues.lhs + 8
    const rhsVal = 24
    if (lhsVal === rhsVal) {
      {
        return {
