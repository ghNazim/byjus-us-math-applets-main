...
const AppInternal: React.FC = () => {
  ...
  const area = useMemo(() => 6 * edgeLength * edgeLength, [edgeLength])
  ...
  const onChange = (value: number) => {
    ggbApi.current?.setValue('l', value)
    setEdgeLength(value)
  }
  return (
    <>
      <Border>
        <CenteredGGB materialId={'yg8eeyvs'} onApiReady={onGGBLoaded} />
      </Border>
      <TickerBox>
      ...
      <MathContent>
        <Math displayMode={true}>
          {String.raw`
          \begin{align*}
          \text{Total surface area} &= \text{6 x (Area of one square face)} \\
          &= \text{6 x \colorbox{#F1EDFF}{\textcolor{#7F5CF4}{$(${edgeLength})^2$}} sq. units } \\
          &= \text{6 x \colorbox{#F1EDFF}{\textcolor{#7F5CF4}{$${edgeLengthSquared}$}} sq. units } \\
...
export const Applet01001Ge: React.FC<{
  ...
  return (
    ...
    >
      <TextCallout
        ...
        text={
          'Change the side length of the cube to understand the calculation of total surface area'
        }
      />
      <AppInternal />
    </AppletContainer>
