...
const EquationHolder = styled.div`
  display: flex;
  align-items: center;
  justify-content: center;
  height: 100px;
...
export const AppletG08EEC09S2GB01: FC<{
  ...
  function handleInput(type: string, ans?: boolean): void {
    switch (type) {
      case 'intercept':
        if (answerState === 2 && !ans) setPageNum(1)
        else if (ans) setPageNum(3)
        else setPageNum(2)
    }
  }

  function handleSlopeInput(ans: boolean): void {
    if (answerState === 2 && !ans) setPageNum(4)
      ...
      {pageNum < 7 ? (
        <NonGGBLayout>
          {showEquation || showSlope ? (
            <Container>
              {showEquation ? <EquationHolder>{question.questionExp}</EquationHolder> : null}
              {showSlope ? (
                <>
                  <SlopeFormer
              ...
              {showIntercept ? (
                <Intercept
                  ...
                  interceptVal={question.intercept}
                />
              ) : null}
            </Container>
          ) : null}
        ...
        <GGBLayout>
          ...
          <InfoHolder
            ...
            yIntercept={question.intercept}
            slopeVals={{ numerator: question.numerator, denominator: question.denominator }}
          />
