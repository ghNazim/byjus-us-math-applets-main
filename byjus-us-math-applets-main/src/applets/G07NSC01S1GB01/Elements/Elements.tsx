  ...
  const sideFinder = (side: 'first' | 'second') => {
    if (id != 2) {
      switch (side) {
        ...
        case 'second':
          return parseFloat(stepperVals.b) < 0 ? 'left' : 'right'
      }
    } else {
      switch (side) {
        case 'first':
        ...
        case 'second':
          return parseFloat(stepperVals.c) / parseFloat(stepperVals.d) < 0 ? 'left' : 'right'
      }
    }
  }
  ...
  const texts: Record<string, ReactElement> = {
    ...
    4: (
      <>
        ...
        {typeof a === 'string' ? (
          <ColorText qnId={id} colorBlock={blueBlock} val={<>{Math.abs(Number(a))}</>} />
        ) : (
          <ColorText
            qnId={id}
    ...
    7: (
      <>
        ...
        units towards the {sideFinder('second')} side.
      </>
    ), //7
...
export const ToolTip: React.FC<{
  ...
  return (
    <>
      ...
      <TextSpan left={314}>
        <DivisionHolder numerator={originalVals.c} denominator={originalVals.d} />=
        {formedDenominator}
      </TextSpan>
    </>
  )
