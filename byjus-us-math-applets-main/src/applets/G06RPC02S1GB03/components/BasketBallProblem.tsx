...
`
const WHITE = 255
const findBLueRgbAccordingToRatio = (numOfBlue: number, targetRgbArr: number[]) => {
  const BLUE_R = (255 - targetRgbArr[0]) / numOfBlue

  const BLUE_G = (255 - targetRgbArr[1]) / numOfBlue

  const BLUE_B = (255 - targetRgbArr[2]) / numOfBlue
...
const BasketBallProblem: FC<{
  ...
  const { BLUE_B, BLUE_G, BLUE_R } = findBLueRgbAccordingToRatio(
    initialBlueCount * randomMultiplier,
    [95, 135, 210],
  )
  ...
  const colorMixing = (numOfWhite: number, numOfBlue: number) => {
    const r = 255 - numOfBlue * BLUE_R
    const g = 255 - numOfBlue * BLUE_G
    const b = 255 - numOfBlue * BLUE_B
    return `rgb(${r},${g},${b})`
  }
