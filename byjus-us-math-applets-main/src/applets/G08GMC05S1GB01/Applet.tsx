...
export const AppletG08GMC05S1GB01: FC<{
  ...
  function handleSliderChange(value: number): void {
    ggb?.setValue('tr', (Math.PI * value) / 100)
  ...
  return (
    ...
    >
      <TextHeader
        text="Explore alternate interior angles."
        backgroundColor="#F6F6F6"
        buttonColor="#1A1A1A"
      />
      <GGBcontainer materialId="tsapdk8m" onApiReady={onGGBLoaded} width={693} height={533} />
      {0 < angleShow && angleShow < 5 && <Slider onChangePercent={handleSliderChange} />}

      {angleShow == 0 && lab == 0 && (
        <Feedback>
          Set the transverse and proceed to view <StyledSpan>alternate interior angles</StyledSpan>.
        </Feedback>
      )}
      {angleShow == 5 && lab == 1 && (
        <Feedback>
          Intersection of parallel lines with a transversal <br />
          forms two pairs of <StyledSpan>alternate interior angles</StyledSpan>.
        </Feedback>
      )}
      {angleShow == 5 && lab == 0 && (
        <Feedback>Select any alternate interior angle and proceed.</Feedback>
      )}
      {!end && angleShow > 0 && angleShow < 5 && (
        <Feedback>
          Overlap the selected alternate interior angles
          <br /> to verify if they are equal.{' '}
        </Feedback>
      )}
      {end && angleShow > 0 && angleShow < 5 && (
        <Feedback>
          The pair of <StyledSpan>alternate interior angles</StyledSpan> are equal.{' '}
        </Feedback>
      )}
      {button == 2 && (
        <CTAButton
