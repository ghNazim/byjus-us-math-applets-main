...
const Patch = styled.div`
  ...
  background-color: #fff;
`
export const AppletG07GMC02S1GB04: FC<{
    ...
    (api: GeogebraAppApi | null) => {
      ...
      ggbApi.current.registerClientListener((e) => {
        ...
        if (
          ...
          e.type === 'mouseDown' &&
          (e.hits[0] === 'side3' || e.hits[0] === 'side4' || e.hits[0] === 'side5')
        ) {
          playClick()
          interaction('tap')
          setShowClick(false)
  ...
  const angle=frame==13?ggbApi.current?.getValue('Angle(B,A,C)')||0:frame==15?ggbApi.current?.getValue('Angle(B+(3,0),B,C)')||0:0;
  useEffect(() => {
  ...
  return (
    <AppletContainer
      {...{
        aspectRatio: 0.9,
        borderColor: '#F6F6F6',
        id: 'g07-gmc02-s1-gb04',
        onEvent,
        className,
      }}
    >
      <TextHeader
        text="Construct a triangle with sides of lengths 3 cm, 4 cm, and 5 cm."
        backgroundColor="#F6F6F6"
        buttonColor="#1A1A1A"
      />
      <Ggb materialId="nakcue9m" onApiReady={onGGBHandle} />
      <Patch />
      <HelperText
        bottom={frame == 2 || frame == 5 ? 200 : frame == 16 ? 130 : 165}
        height={frame == 16 ? 120 : 60}
      >
        {ggbLoaded && frame == 1 && <Text>{'Follow the steps given for construction of the triangle.'}</Text>}
