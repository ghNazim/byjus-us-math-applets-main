...
export const Applet4505Ge: React.FC<{
  ...
  const onSliderChange = (value: number) => {
    ...
    ggbApiRight.current.setValue('t', 1 - value / 100)
    if (value === 100) {
      setShowHands((v) => {
        return v == 0 || v == 1 ? 1 : 2
      })
  ...
  useEffect(() => {
    ...
    ggbApiRight.current.setValue('Options', option)
    ggbApiLeft.current.setValue('Options', option)
  }, [activeToggle])
  return (
    <AppletContainer
      {...{
    ...
    >
      <TextHeader
        text="Unfold the triangular prism and observe its net."
      ...
      {ggbLoaded && (
        <ToggleGroup noOfChildren={6} onChange={handleToggleChange} disabled={resetSlider} />
      )}
