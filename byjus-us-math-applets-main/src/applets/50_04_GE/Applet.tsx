...
export const Applet5004Ge: React.FC<{
  ...
  const onViewdirectionHandle = () => {
    if (ggbApiLeft.current) {
      ggbApiLeft.current.evalCommand('SetViewDirection(Vector((.8, .8, -.2)))')
    }
  }
  ...
  const onChangeLeftTicker = (value: number) => {
    if (ggbApiRight.current && ggbApiLeft.current) {
      ggbApiLeft.current.setVisible('j', false)
      ggbApiLeft.current.setVisible('zA', true)
      ggbApiLeft.current.evalCommand(`SetValue(tt1,${value}°)`)
      ggbApiRight.current.evalCommand(`SetValue(tt1,${value}°)`)
      if (value !== 0)
        setOnboardingAnimation((p) => {
          const d = { ...p }
          d[1] = false
  ...
  return (
    ...
    >
      <TextHeader
        text="Move the plane at different angles and view the cross&nbsp;sections of the right rectangular pyramid."
