 ...
 * @param rootNode The node we are starting the walk from
 * @param options Options for the positioning
 * @return {boolean} true if the tree fits and false if it does not
 */
export default function positionTree(tree: Graph, rootNode: GraphNode, options: Options): boolean {
  if (tree.hasNode(rootNode)) {
    // Do the preliminary positioning with a postorder walk
    firstWalk(tree, rootNode, 0, options)
    // Determine how to adjust all the nodes with respect to
    // the location of the root.
    tree.xTopAdjustment = tree.xCoord(rootNode) - tree.prelim(rootNode)
    tree.yTopAdjustment = tree.yCoord(rootNode)
    return secondWalk(tree, rootNode, 0, 0, options)
  }
  return true
}
 ...
 * @param options Options for the positioning
 */
export function firstWalk(tree: Graph, node: GraphNode, level: number, options: Options) {
  ...
  if (tree.isLeaf(node) || level === options.maxDepth) {
    ...
    const leftMost = tree.firstChild(node)
 ...
 * @param options Options for the positioning
 */
export function apportion(tree: Graph, node: GraphNode, level: number, options: Options) {
  let leftMost = tree.firstChild(node)
  // THIS SHOULD BE C BUT IT IS STILL D
  let neighbor = tree.leftNeighbor(leftMost)
  let compareDepth = 1
  const depthToStop = options.maxDepth - level
  ...
  while (leftMost !== null && neighbor !== null && compareDepth <= depthToStop) {
    ...
    let leftModsum = 0
    let rightModsum = 0
    let ancestorLeftMost: GraphNode = leftMost
    let ancestorNeighbor: GraphNode = neighbor
 ...
 */
export function getLeftMost(tree: Graph, node: GraphNode, level: number, depth: number): GraphNode {
...
export function secondWalk(
  ...
  if (level <= options.maxDepth) {
    const xTemp = tree.xTopAdjustment + tree.prelim(node) + modSum
    const yTemp = tree.yTopAdjustment + level * options.levelSeparation
    ...
    if (checkExtendsRange(xTemp, yTemp, options)) {
      ...
      if (result && tree.hasRightSibling(node)) {
        result = secondWalk(tree, tree.rightSibling(node), level, modSum, options)
      }
    } else {
 ...
 * @param options Options for the positioning
 */
export function checkExtendsRange(x: number, y: number, options: Options): boolean {
